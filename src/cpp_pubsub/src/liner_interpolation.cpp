...
double NormalizeAngle_(const double angle)
{
...
double Liner_Interpolation::slerp(const double a0, const double t0, const double a1, const double t1,
                                  const double t)
{
    // if (std::abs(t1 - t0) <= kMathEpsilon)
    // {
    //     ADEBUG << "input time difference is too small";
    //     return NormalizeAngle_(a0);
    // }
    const double a0_n = NormalizeAngle_(a0);
    const double a1_n = NormalizeAngle_(a1);
    double d = a1_n - a0_n;
    if (d > M_PI)
    {
        d = d - 2 * M_PI;
    ...
    return NormalizeAngle_(a);
}

SLPoint Liner_Interpolation::InterpolateUsingLinearApproximation(const SLPoint &p0,
...
PathPoint Liner_Interpolation::InterpolateUsingLinearApproximation(const PathPoint &p0,
                                                                   const PathPoint &p1,
                                                                   const double s)
{
    double s0 = p0.s;
    ...
    // cout << " s: " << s << " s0: " << s0 << " s1: " << s1 << " x: " << x << " y: " << y << " p0.x: " << p0.x << " p0.y: " << p0.y << endl;
    ...
    path_point.s = s;
    // cout << " x: " << path_point.x << " y: " << path_point.y << " s: " << path_point.s << endl;
    return path_point;
}

// TrajectoryPoint Liner_Interpolation::InterpolateUsingLinearApproximation(const TrajectoryPoint &tp0,
